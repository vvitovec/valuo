...
registry = json.loads(
    (Path(os.environ["HOUSESPREDICT_ARTIFACTS_DIR"]) / "model-registry.json").read_text(encoding="utf-8")
)
...
summary = {
    ...
    "promoted": bool(latest_entry.get("promoted")),
    "promotionReason": latest_entry.get("promotionReason"),
...
if summary.get("qualityStatus") == "degraded":
    print("degraded")
else:
    print("success")
PY
